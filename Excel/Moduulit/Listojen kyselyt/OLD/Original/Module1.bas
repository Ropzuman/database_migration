...
'Tuhotaan DB-sheetit, jotta ne saadaan varmasti tyhj‰ksi
'Sheets("DB1").Delete
'Sheets("DB2").Delete
  ...
  For i = 1 To 2
    ...
    If sSQL(i) <> "" Then
      ...
      With TAULUKKO
        ...
        .Refresh
        .Delete 'Tuhotaan query lopuksi, jottei se j‰‰ tyˆkirjaan roikkumaan
      End With
...
Sub GenPrintout()
  ...
  Else 'Tiedot on analysoitu ja voidaan alkaa viem‰‰n tietoja uuteen sheetiin.
    ...
    Sheets("DB1").Select
    'Recordeja = ActiveCell.SpecialCells(xlLastCell).Row
    ...
    MacroWB = ActiveWorkbook.Name ' Otetaan ylˆs tyˆkirjan nimi
    'Kopioidaan Infosivu ensin uuteen tyˆkirjaan
    Sheets("Info").Copy      'Kopioi ja muodostaa uuden tyˆkirjan
    UusiWB = ActiveWorkbook.Name 'Otetaan ylˆs uudentyˆkirjan nimi
    'Poistetaan kommentit
    Cells.ClearComments
    'Kopioidaan sitten Template uuteen tyˆkirjaan
    Windows(MacroWB).Activate
    Sheets("TEMPLATE").Copy After:=Workbooks(UusiWB).Sheets(1)
    ActiveSheet.Name = POSheet   'Muutetaan Sheetin nimi
    
    
    'Kopioidaan sitten Legend ja Revisions sheetit uuten tyˆkirjaan
    Windows(MacroWB).Activate
    Sheets("Legend").Copy After:=Workbooks(UusiWB).Sheets(2)
    Windows(MacroWB).Activate
    Sheets("Revisions").Copy After:=Workbooks(UusiWB).Sheets(1)
    Cells.ClearComments
        
    'Kopioidaan tietokanta omalle sheetilleen
    Sheets.Add
    ActiveSheet.Name = "LINKING"
    ActiveSheet.Move After:=Sheets(Sheets.Count)
    Windows(MacroWB).Activate
    Sheets("DB1").Cells.Copy
    Windows(UusiWB).Activate
    Sheets("LINKING").Paste
    Range("A1").Select
    Sheets(POSheet).Select
    'Poistetaan Printout sheetilt‰ kaikki tiedot, koska nyt valitaan vasta mit‰ sinne vied‰‰n
    Cells.Clear 'Poistetaan tekstit ym. (solujen leveydet s‰ilyv‰t, mik‰ on tarkoitus
    If ActiveSheet.Shapes.Count > 0 Then 'Poistetaan kaikki mahdolliset kuvat ym.
    ...
    ViimRivi = 1
    Windows(MacroWB).Activate
    Sheets("TEMPLATE").Rows(PHStart & ":" & PHEnd).Copy 'Kopioidaan TEMPLATE sheetilt‰ data
    Windows(UusiWB).Activate
    Rows(ViimRivi & ":" & ViimRivi + PHEnd - PHStart).Select
    ActiveSheet.Paste 'Liitet‰‰n UUTEEN Workbookiin (Liitt‰‰ muotoilut kuvat jne.)
      
    ActiveSheet.PageSetup.PrintTitleRows = "$" & ViimRivi & ":$" & ViimRivi + PHEnd - PHStart 'M‰‰ritell‰‰n otsikkorivit
...
'---------- [ ALATUNNISTEET ] --------------------------
    ...
    Sarjoja = 0
    For i = 2 To Recordeja Step RMAX 'Sitten kopioidaan loput tiedot
      Application.StatusBar = "Prosessing record: " & i & "/" & Recordeja
      Windows(MacroWB).Activate
      Sheets("TEMPLATE").Rows(DocStart & ":" & DocEnd).Copy 'Kopioidaan TEMPLATE sheetilt‰ data
      Windows(UusiWB).Activate
      Rows(ViimRivi & ":" & ViimRivi + Riveja).Select
      ActiveSheet.Paste  'Liitet‰‰n UUTEEN Workbookiin (Liitt‰‰ muotoilut linkit jne.)
      If i Mod 2 = 1 Then
    ...
    If AddFooter = True Then
        Windows(MacroWB).Activate
        Sheets("TEMPLATE").Rows(PFStart & ":" & PFEnd).Copy 'Kopioidaan TEMPLATE sheetilt‰ data
        Windows(UusiWB).Activate
...
'    End If
    ...
    If Tiedosto <> "" Then
'      ActiveWorkbook.SaveAs DIPath & DIDocNo & ".xls"
'      ActiveWorkbook.SaveAs Tiedosto, xlExcel12
      ActiveWorkbook.BuiltinDocumentProperties("Title").Value = POSheet
      ActiveWorkbook.SaveAs Tiedosto, xlOpenXMLWorkbook
    End If
  End If
End Sub
Sub Checkout() 'T‰m‰ tarkistaa lˆytyv‰tkˆ kaikki otsikot datasta
Dim i As Integer
Dim j As Integer
Dim Arvo As String
...
'   HideLINKING = Sheets("Main").HLINKING.Value
   ...
   HaeDocTiedot 'Hakee dokumentin tiedot DB2-sheetilt‰
   VaihdaInfo   'Vaihtaa dokumentin tiedot info sheetille
   VaihdaInfo ("Revisions") 'Tiedot revisions sheetille
   
   Sheets("TEMPLATE").Select
   'Haetaan ensin kerralla kopioitavien rivien m‰‰r‰ eli rivitysmerkinn‰t
   ...
   If Virhe Then
     ...
     Exit Sub
   End If
   'Rivitys merkinn‰tolivat oikein, etsit‰‰n otsikoita
   For i = DocStart To DocEnd
     For j = 1 To Sarakkeita
       Arvo = Cells(i, j).Value
       If Len(Arvo) > 2 Then 'Solussa on tietoa
